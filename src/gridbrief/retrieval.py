...
def lexical_search(
    ...
    """Bounded low-memory retrieval for constrained web instances.

    Embeddings remain generated by the indexing worker. The public web process
    avoids loading PyTorch and ranks a small SQL candidate set by query-term
    overlap instead.
    """
    if k <= 0 or k > MAX_RESULT_COUNT:
        raise ValueError(f"k must be between 1 and {MAX_RESULT_COUNT}")
    normalized_query = _normalize_query(query)
    terms = sorted(
        {term.lower() for term in normalized_query.split() if len(term) >= 3},
        key=len,
        reverse=True,
    )[:8]
    if not terms:
    ...
    active_filters = filters or SearchFilters()
    iso = active_filters.iso or get_settings().iso
    statement = (
    ...
    if active_filters.published_before is not None:
        statement = statement.where(Chunk.published_at <= active_filters.published_before)
    statement = statement.order_by(Chunk.published_at.desc(), Chunk.chunk_id.desc()).limit(k * 6)
    with get_session_factory()() as session:
        rows = session.execute(statement).all()
    ranked = []
    for chunk, title in rows:
        haystack = f"{title or ''} {chunk.text}".lower()
        hits = sum(haystack.count(term) for term in terms)
        score = hits / max(1, len(terms))
        ranked.append((score, chunk, title))
    ranked.sort(
        key=lambda item: (
            item[0],
    ...
    return [
        ...
        for score, chunk, title in ranked[:k]
    ]
...
def vector_search(
    ...
    if get_settings().retrieval_backend == "lexical":
        return lexical_search(query=query, filters=filters, k=k)
    return search(
        query=query,
        filters=filters,
        k=k,
